...
\item In Step 8 \button{Test Cluster Setup}, some tests may fail for the first
  ...
  tests succeeded.  If APItests are failing, it may be because the OSCAR
  Wizard did not have the correct environment variables set.  The way around
  this problem is to quit the Wizard, open another terminal, bring up the
  Wizard and re-run the tests.  All tests (including APItests) should pass
  if the cluster is configured correctly.
