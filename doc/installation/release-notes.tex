...
\item The pfilter package may cause image deployment to freeze midway through,
  ...
  \cmd{/etc/init.d/pfilter stop}.  OSCAR will restart the service during
...
\item When using the \button{Install/Uninstall OSCAR Packages} feature, a RPM
  cache needs to be first initialized.  This will only take a few minutes and
  only happens the first time this functionality is used.
...
% TJN: Adding in response to Bug#1271143
\item On Red Hat Enterprise Linux 4 Update 2+ and CentOS 4.2+ systems you 
      ...
      server, if installed, to avoid file ownership conflicts with `pvm' on
      the compute nodes and SIS image.  Use the command:

      \begin{verbatim}
      # rpm --erase tog-pegasus tog-pegasus-devel
      \end{verbatim}

      This is due to an ordering issue with the RPM installs which 
...
\file{redhat-3asU6-x86\_64.rpmlist} otherwise the build process will error
...
the image has been created by executing the \file{rhel3u6\_fixup} script
located in the \file{\$OSCAR\_HOME/scripts} directory.  This will install the
