...
\item If you downloaded PBS through Step 0 \button{Download Addtional 
  ...
  Step 1 \button{Select OSCAR Packages To Install}, select the
...
\file{/tftpboot/rpm} before starting the OSCAR Wizard.  The easiest way is to
...
\item Red Hat Enterprise Linux 3 (Gold or U2) rpmlist.
...
OSCAR Wizard (step 4), copy a valid initrd to \file{/boot/efi/EFI/redhat} on
the image:
\begin{verbatim}
...
SR870BH2, you need to add the USB controller to \file{/etc/modules.conf} of
...
directory you must install the package and turn it on:
    ...
    \cmd{rpm -i tftp-server-0.32-4.ia64.rpm}

(it could be "ia64" or "i386", depending of your machine's architecture).

    \cmd{/sbin/chkconfig --level 345 tftp on}
...
kernel command line:
    ...
    \cmd{DISKORDER=hd,sd,cciss,ida,rd} 

For i386 clients, add the value to the ``APPEND'' keyword in

    \cmd{/tfpboot/pxelinuf.cfg/default}
...
%dnl\item elilo.
%dnl
%dnlIf Linux doesn't properly boot after the installation of the image,
%dnlyou may have to manually boot it from the correct path.  Choose the
%dnl``EFI Shell'' boot option (SR870BH2), then access the correct device (in
%dnlour case fs0), copy elilo.efi and elilo.conf to the root: 
%dnl
%dnl\begin{verbatim}
%dnl   cp  \EFI\redhat\elilo.efi   \
%dnl   cp  \EFI\redhat\elilo.conf  \ 
%dnl\end{verbatim}
%dnl
%dnl%TJN: Can someone confirm the path is correct and my footnote is accurate.
%dnland boot from there ("elilo.efi 2.4.21-15.EL")~\footnote{Note the backward
%dnlslashes for the EFI shell.}.
